...
export function CategoriesManager() {
  const { categories, setCategories, loading } = useCategories(true);
  ...
  async function handleCreate(event: React.FormEvent) {
    ...
    try {
      ...
      toast.error(errMsg(error, "Couldn't add category"));
    } finally {
      setCreating(false);
    }
  }
  ...
  async function handleRename(id: string) {
    ...
    try {
      ...
      toast.error(errMsg(error, "Rename failed"));
    } finally {
      setBusyId(null);
    }
  }
  ...
  async function handleDelete() {
    ...
    try {
      await apiFetch(`/categories/${pendingDelete.id}`, { method: "DELETE" });
      setCategories((prev) => prev.filter((c) => c.id !== pendingDelete.id));
      toast.success(`Deleted "${pendingDelete.name}"`);
    } catch (error) {
      toast.error(errMsg(error, "Delete failed"));
    } finally {
      setBusyId(null);
      ...
      <Dialog open={Boolean(pendingDelete)} onOpenChange={(open) => !open && setPendingDelete(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Photos already assigned to this category will become uncategorised. This can&apos;t
              be undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
