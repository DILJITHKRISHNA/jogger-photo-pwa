...
export function ExcelUploadPanel({
  ...
  type: Exclude<ImportType, "photos">;
  title: string;
  columnsHint: string;
  ...
  useEffect(() => {
    fetchJson<ImportRecord[]>(`/api/admin/import-history?type=${type}`)
  ...
  async function handleUpload() {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/excel/${type}`, {
        method: "POST",
        body: formData,
      });
      ...
      setLastRecord(data as ImportRecord);
      ...
      <div className="rounded-2xl border border-border bg-card p-4 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <Button
            ...
            size="sm"
            nativeButton={false}
            render={<a href={`/api/admin/excel/template?type=${type}`} />}
          >
            <Download /> Template
          </Button>
        </div>
          ...
          <div className="mt-4 flex flex-col gap-3">
              ...
              {lastRecord.errorCount > 0 && (
                <AlertDescription className="flex flex-col gap-2">
                  ...
                  <a
                    ...
                    href={`/api/admin/import-history/${lastRecord.id}/errors`}
                  >
            ...
            {lastRecord.missingPhotos.length > 0 && (
              <Alert variant="warning">
                ...
                <AlertDescription>
                  <p className="mb-1">These won&apos;t appear for executives until a matching photo is uploaded:</p>
