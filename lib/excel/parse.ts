 ...
 * Invalid rows are reported as errors, never silently imported — matching
 * the "do not silently import invalid rows" rule from the spec.
 */
export function parseProductExcel(buffer: Buffer): ParsedExcelResult {
  ...
  if (!header) {
    return {
      ...
      errors: [],
      fatal:
        'Could not find "Article" and "Colour" columns. Check the header row and try again.',
    };
  }
  ...
  for (let r = header.index + 1; r < matrix.length; r++) {
    ...
    total += 1;
    const rowNumber = r + 1; // human-friendly, matches spreadsheet row number
