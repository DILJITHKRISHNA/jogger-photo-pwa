import { normalizeArticle, normalizeColour } from "@/lib/product-key";
...
export function parsePhotoFilename(filename: string): ParsedPhotoFilename {
  ...
  // Prefer the space separator the spec calls for; fall back to underscore/hyphen
  // for real-world exports so we still recover a usable Article + Colour pair.
...
export function fileSafeKey(article: string, colour: string): string {
  return `${normalizeArticle(article)}_${normalizeColour(colour)}`.replace(
    /[^A-Z0-9_]+/gi,
    "-"
  );
}
